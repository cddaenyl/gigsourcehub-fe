...
watch(
  () => aiSearchMutation.isSuccess.value,
  async (success) => {
  ...
  <div class="flex items-center gap-3">
      ...
      <template #icon>
        <n-icon>
          <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path
              d="M19 19L19.6 20.4L21 21L19.6 21.6L19 23L18.4 21.6L17 21L18.4 20.4L19 19Z"
              fill="currentColor"
            />
          </svg>
        </n-icon>
    ...
    <n-drawer v-model:show="active" :width="500" placement="right">
      ...
      <div class="h-full w-full flex flex-col bg-white font-sans overflow-hidden">
        ...
        <div
          class="flex items-center justify-between p-5 border-b border-slate-100 shrink-0 shadow-[0_4px_10px_-10px_rgba(0,0,0,0.1)] z-10 bg-white relative"
        >
          ...
          <div class="flex items-center space-x-2">
            ...
            <button
              @click="active = false"
              class="w-9 h-9 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-500 border border-slate-200 transition-colors"
            >
        ...
        <div class="flex-1 overflow-hidden relative flex flex-col">
          ...
          <Transition name="fade">
            <div
              v-show="showHistory"
              class="absolute inset-0 bg-white z-20 flex flex-col p-5 overflow-y-auto space-y-3"
            >
              <div class="flex items-center justify-between mb-4">
                ...
                <span
                  class="text-[11px] text-slate-400 bg-slate-50 px-2 py-0.5 rounded-full border border-slate-100"
                >
              ...
              <div v-else class="space-y-2">
                <div
                  ...
                  :class="{
                    'border-blue-400 bg-blue-50/50 ring-2 ring-blue-500/5':
                      currentChatId === chat.id,
                  }"
                  @click="selectChat(chat.id)"
                >
                  <div
                    class="w-9 h-9 rounded-lg bg-slate-100 flex items-center justify-center mr-3 shrink-0 group-hover:bg-blue-100 transition-colors"
                  >
                    <n-icon
                      :component="ApiApp"
                      size="18"
                      class="text-slate-500 group-hover:text-[#0014B2]"
                    />
                  </div>
          ...
          <div ref="chatContainerRef" class="flex-1 overflow-y-auto px-5 py-6 space-y-7">
            ...
            <div
              ...
              class="h-full flex flex-col items-center justify-center py-20 text-center space-y-5 px-10"
            >
              <div
                class="w-16 h-16 rounded-3xl bg-blue-50 border border-blue-100 flex items-center justify-center shadow-sm"
              >
                <svg
                  width="32"
                  height="32"
              ...
              <div class="space-y-2">
                ...
                <p class="text-sm text-slate-500 leading-relaxed">
                  Cari kandidat berdasarkan kriteria teknis, lokasi, atau pengalaman dengan bahasa
                  natural.
                </p>
              </div>
              <div class="flex flex-wrap items-center justify-center gap-2 pt-2">
                <button
                  @click="
                    ((queryInput = 'Cari Frontend Developer di Jakarta'), handleSendMessage())
                  "
                  class="px-4 py-2 rounded-full border border-slate-200 text-xs font-semibold text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all"
                >
                  "Cari Frontend Developer di Jakarta"
                </button>
                <button
                  @click="
                    ((queryInput = 'Kandidat yang ahli React dan Node.js'), handleSendMessage())
                  "
                  class="px-4 py-2 rounded-full border border-slate-200 text-xs font-semibold text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all"
                >
            ...
            <div v-for="msg in chatHistory" :key="msg.id" class="flex flex-col w-full">
              ...
              <div v-if="msg.role === 'ai'" class="flex w-full justify-start space-x-4 mt-2">
                <div
                  class="shrink-0 w-10 h-10 rounded-full bg-[#E0E7FF] flex items-center justify-center shadow-sm"
                >
                  <svg
                    width="20"
                    height="20"
                  ...
                  >
                    ...
                    />
                    <path
                      d="M5 4L5.6 5.4L7 6L5.6 6.6L5 8L4.4 6.6L3 6L4.4 5.4L5 4Z"
                      fill="currentColor"
                    />
                  </svg>
                </div>
                ...
                <div class="flex flex-col w-full max-w-[90%]">
                  <!-- Summary Text -->
                  <div
                    class="bg-white border border-slate-100 px-6 py-5 rounded-[20px] rounded-tl-sm text-[15px] text-slate-600 leading-relaxed shadow-sm relative group whitespace-pre-wrap"
                  >
                    {{ msg.text }}
                    <div v-if="msg.isTyping" class="inline-flex items-center ml-2 space-x-1">
                      ...
                      <span
                        class="w-1 h-1 bg-blue-400 rounded-full animate-bounce [animation-delay:-0.15s]"
                      ></span>
                      <span
                        class="w-1 h-1 bg-blue-400 rounded-full animate-bounce [animation-delay:-0.3s]"
                      ></span>
                    </div>
                  </div>
                  ...
                  <Transition name="slide-up">
                    <div
                      v-if="
                        msg.displayParsedContent &&
                        msg.parsedContent &&
                        msg.parsedContent.candidates &&
                        msg.parsedContent.candidates.length > 0
                      "
                      class="mt-7 w-full overflow-visible"
                    >
                      <div
                        class="text-[13px] font-bold text-slate-700 mb-3 px-1 tracking-wide uppercase flex items-center space-x-2"
                      >
                      ...
                      <div class="space-y-3.5">
                        ...
                        >
                          <div class="flex items-center space-x-4">
                            <div
                              class="w-12 h-12 rounded-lg bg-blue-50 border border-blue-100 flex items-center justify-center overflow-hidden shrink-0 shadow-sm relative"
                            >
                              <img
                                v-if="profilePics[cand.id]"
                                :src="profilePics[cand.id]!"
                            ...
                            <div class="flex flex-col space-y-1">
                              <div class="flex items-center gap-2">
                                <span
                                  class="font-semibold text-[14px] text-slate-700 truncate group-hover:text-[#0014B2] transition-colors"
                                  >{{ cand.name }}</span
                                >
                                <CandidateLevelChip
                              ...
                              <span
                                class="bg-blue-50 text-[#0014B2] px-1.5 py-0.5 rounded text-[10px] font-bold border border-blue-100 self-start"
                                >{{ cand.score }}% Match</span
                              >
                            </div>
                          </div>

                          <div
                            class="flex items-center justify-center w-6 h-6 text-slate-400 group-hover:text-[#0014B2] group-hover:translate-x-0.5 transition-all"
                          >
            ...
            <div v-if="isLoading" class="flex w-full justify-start space-x-4 mt-2">
              ...
              <div
                class="bg-white border border-slate-100 px-5 py-3 rounded-[20px] rounded-tl-sm shadow-sm flex items-center space-x-3 w-24 h-12"
              >
                <n-spin size="small" />
              </div>
            </div>
        ...
        <div
          class="h-6 w-full bg-linear-to-b from-transparent to-white pointer-events-none -mt-6 z-10"
        ></div>

        <!-- Footer / Input Section -->
        <div
          class="bg-white p-5 border-t border-slate-100 shrink-0 z-20 w-full flex items-center justify-center shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.02)]"
        >
          <!-- Text Input (Full width rounded pill) -->
          <div
            class="flex items-end gap-2 w-full bg-white border border-slate-300 hover:border-slate-400 focus-within:border-[#0014B2] focus-within:ring-4 focus-within:ring-blue-600/10 rounded-[24px] pl-5 pr-2 py-2 shadow-sm transition-all"
          >
            <textarea
              ref="textareaRef"
              v-model="queryInput"
