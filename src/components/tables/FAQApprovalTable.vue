...
function parseProposed(proposedStr?: string | null): { question: string; answer: string } {
  if (!proposedStr) return { question: '—', answer: '—' }
  try {
...
const columns: DataTableColumns<ApprovalRequest> = [
  ...
  {
    title: 'Pertanyaan (Proposed)',
    key: 'question',
    ellipsis: { tooltip: true },
    render: (row) => {
      const parsed = parseProposed(row.proposed_data)
      return h('span', { class: 'text-slate-700 font-medium text-sm' }, parsed.question)
    },
  },
  {
    title: 'Jawaban (Proposed)',
    key: 'answer',
    ellipsis: { tooltip: true },
    render: (row) => {
      const parsed = parseProposed(row.proposed_data)
      return h('span', { class: 'text-slate-600 text-sm' }, parsed.answer)
    },
  },
  {
    title: 'Pemohon',
