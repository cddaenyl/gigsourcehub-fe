  ...
  const connect = async () => {
    ...
    try {
      ...
      if (isTokenExpired(authStore.token)) {
        console.log('Token is expired, attempting to refresh token before connecting WebSocket...')
        if (authStore.refreshToken) {
          try {
            const { refreshTokenApi } = await import('@/services/auth.service')
            const res = await refreshTokenApi({ refresh_token: authStore.refreshToken })
            if (res && res.data && res.data.token) {
              authStore.setAuth(res.data.token, res.data.user, res.data.refresh_token)
            ...
            console.error('Failed to refresh token for WebSocket, logging out...', err)
            authStore.logout()
            return
          }
        } else {
