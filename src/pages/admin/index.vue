    ...
    <div class="space-y-6 max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div v-if="isLoading" class="space-y-6">
        ...
        <n-grid cols="1 s:2 m:3 l:5" responsive="screen" :x-gap="16" :y-gap="16">
          <n-gi v-for="i in 5" :key="i">
            <div
              class="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm space-y-3 animate-pulse"
            >
            ...
            <n-card class="shadow-sm rounded-xl border border-slate-100 animate-pulse">
              ...
              <n-grid cols="2 m:4" :x-gap="16" :y-gap="16">
                <n-gi v-for="i in 4" :key="i">
                  <div
                    class="p-4 bg-slate-50 rounded-xl border border-slate-100 text-center space-y-2"
                  >
            ...
            <n-card class="shadow-sm rounded-xl border border-slate-100 animate-pulse">
              ...
              <div class="space-y-3">
                <div
                  v-for="i in 4"
                  :key="i"
                  class="flex items-center justify-between p-3.5 rounded-xl border border-slate-100 bg-slate-50/50"
                >
                  <div class="flex items-center gap-3">
      ...
      <template v-else-if="dashboardData">
        ...
        <n-grid cols="1 s:2 m:3 l:5" responsive="screen" :x-gap="16" :y-gap="16">
          <n-gi>
            <div
              class="relative overflow-hidden bg-gradient-to-br from-teal-500 to-emerald-600 rounded-2xl p-5 text-white shadow-md hover:-translate-y-1 transition-all duration-300"
            >
              <div class="absolute right-2 -bottom-2 opacity-15">
                <n-icon :component="Clock" size="72" />
              </div>
              <p class="text-teal-100 text-xs font-semibold uppercase tracking-wider">
                Avg Time-to-Hire
              </p>
              <h3 class="text-3xl font-bold mt-2">
                {{ dashboardData.kpis.time_to_hire_days }}
          ...
          <n-gi>
            <div
              class="relative overflow-hidden bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl p-5 text-white shadow-md hover:-translate-y-1 transition-all duration-300"
            >
              <div class="absolute right-2 -bottom-2 opacity-15">
                <n-icon :component="Check" size="72" />
              </div>
              <p class="text-blue-100 text-xs font-semibold uppercase tracking-wider">
                Offer Acceptance
              </p>
              <h3 class="text-3xl font-bold mt-2">
                {{ dashboardData.kpis.offer_acceptance_rate }}%
              </h3>
            </div>
          </n-gi>
          ...
          <n-gi>
            <div
              class="relative overflow-hidden bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl p-5 text-white shadow-md hover:-translate-y-1 transition-all duration-300"
            >
              <div class="absolute right-2 -bottom-2 opacity-15">
                <n-icon :component="Briefcase" size="72" />
              </div>
              <p class="text-indigo-100 text-xs font-semibold uppercase tracking-wider">
                Published Job Vacancies
              </p>
              <h3 class="text-3xl font-bold mt-2">
                {{ dashboardData.kpis.active_job_vacancies }}
                <span class="text-sm font-normal">Posisi</span>
              </h3>
            </div>
          </n-gi>
          ...
          <n-gi>
            <div
              class="relative overflow-hidden bg-gradient-to-br from-purple-500 to-pink-600 rounded-2xl p-5 text-white shadow-md hover:-translate-y-1 transition-all duration-300"
            >
              <div class="absolute right-2 -bottom-2 opacity-15">
                <n-icon :component="Calendar" size="72" />
              </div>
              <p class="text-purple-100 text-xs font-semibold uppercase tracking-wider">
                Interview Attendance
              </p>
              <h3 class="text-3xl font-bold mt-2">
                {{ dashboardData.kpis.interview_attendance_rate }}%
              </h3>
            </div>
          </n-gi>
          ...
          <n-gi>
            <div
              class="relative overflow-hidden bg-gradient-to-br from-orange-500 to-rose-600 rounded-2xl p-5 text-white shadow-md hover:-translate-y-1 transition-all duration-300"
            >
              <div class="absolute right-2 -bottom-2 opacity-15">
                <n-icon :component="TrendingUp" size="72" />
              </div>
              <p class="text-orange-100 text-xs font-semibold uppercase tracking-wider">
                AVG Review Score
              </p>
              <h3 class="text-3xl font-bold mt-2">
                {{ dashboardData.kpis.avg_review_score }}
        ...
        <n-grid cols="1 l:3" :x-gap="20" :y-gap="20" responsive="screen">
          ...
          <n-gi span="2" class="space-y-6">
            ...
            <n-card
              ...
              class="shadow-sm rounded-xl border border-slate-100"
            >
              ...
              <div
                v-if="dashboardData.upcoming_interviews.length === 0"
              ...
              <div v-else class="divide-y divide-slate-100">
                ...
                >
                  <div class="flex items-center gap-3">
                    <div
                      class="h-10 w-10 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center font-bold text-sm overflow-hidden shrink-0 border border-slate-100"
                    >
                      <img
                        v-if="iv.candidate_user_profile_picture"
                        :src="getProfilePictureThumbnail(iv.candidate_user_profile_picture)"
                        alt="Avatar"
                    ...
                    <div>
                      ...
                      <p class="text-xs text-slate-500">
                        {{ iv.subrequest?.project_name || 'Proyek Tidak Terdefinisi' }} &bull;
                        {{ iv.stage?.name || 'Interview' }}
                      </p>
                    </div>
                  </div>
            ...
            <n-card title="Ringkasan Request" class="shadow-sm rounded-xl border border-slate-100">
              ...
              <div
                class="bg-gradient-to-r from-slate-50 to-blue-50/50 rounded-xl p-5 mb-6 border border-slate-100"
              >
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div class="space-y-1">
                    ...
                    <h4 class="text-xl font-bold text-slate-800">
                      {{ dashboardData.requests_summary.filled_headcount }} dari
                      {{ dashboardData.requests_summary.required_headcount }} Posisi Terpenuhi
                    </h4>
                  </div>
                ...
                <n-gi>
                  <div class="p-4 bg-slate-50/50 rounded-xl border border-slate-100 text-center">
                    ...
                    <h3 class="text-2xl font-bold text-slate-800 mt-1">
                      {{ dashboardData.requests_summary.total_requests }}
                    </h3>
                  </div>
                </n-gi>
                <n-gi>
                  <div class="p-4 bg-slate-50/50 rounded-xl border border-slate-100 text-center">
                    ...
                    <h3 class="text-2xl font-bold text-green-600 mt-1">
                      {{ dashboardData.requests_summary.fulfilled_requests }}
                    </h3>
                  </div>
                </n-gi>
                <n-gi>
                  <div class="p-4 bg-slate-50/50 rounded-xl border border-slate-100 text-center">
                    ...
                    <h3 class="text-2xl font-bold text-orange-600 mt-1">
                      {{ dashboardData.requests_summary.waiting_validation_requests }}
                    </h3>
                  </div>
                </n-gi>
                <n-gi>
                  <div class="p-4 bg-slate-50/50 rounded-xl border border-slate-100 text-center">
                    ...
                    <h3 class="text-2xl font-bold text-blue-600 mt-1">
                      {{ dashboardData.requests_summary.in_progress_requests }}
                    </h3>
                  </div>
                </n-gi>
              </n-grid>
              ...
              <div class="mt-5 flex items-center justify-between border-t border-slate-100 pt-4">
                ...
                <span class="text-xs font-bold text-slate-700">
                  {{ Math.round(dashboardData.requests_summary.request_fulfillment_percentage) }}%
                  Request Selesai
                </span>
              </div>
            </n-card>
          ...
          <n-gi class="space-y-6">
            ...
            >
              <div class="space-y-3">
                <div
                  class="flex items-center justify-between p-3.5 rounded-xl border border-rose-100 bg-rose-50/30"
                >
                  <div class="flex items-center gap-3">
                    <div
                      class="h-9 w-9 rounded-lg bg-rose-100 text-rose-600 flex items-center justify-center"
                    >
                  ...
                  <span class="text-lg font-black text-rose-600">{{
                    dashboardData.alerts.overdue_requests
                  }}</span>
                </div>

                <div
                  class="flex items-center justify-between p-3.5 rounded-xl border border-amber-100 bg-amber-50/30"
                >
                  <div class="flex items-center gap-3">
                    <div
                      class="h-9 w-9 rounded-lg bg-amber-100 text-amber-600 flex items-center justify-center"
                    >
                  ...
                  <span class="text-lg font-black text-amber-600">{{
                    dashboardData.alerts.interviews_scheduled_today
                  }}</span>
                </div>

                <div
                  class="flex items-center justify-between p-3.5 rounded-xl border border-blue-100 bg-blue-50/30"
                >
                  <div class="flex items-center gap-3">
                    <div
                      class="h-9 w-9 rounded-lg bg-blue-100 text-blue-600 flex items-center justify-center"
                    >
                  ...
                  <span class="text-lg font-black text-blue-600">{{
                    dashboardData.alerts.unassigned_requests
                  }}</span>
                </div>

                <div
                  class="flex items-center justify-between p-3.5 rounded-xl border border-slate-200 bg-slate-50/50"
                >
                  <div class="flex items-center gap-3">
                    <div
                      class="h-9 w-9 rounded-lg bg-slate-200 text-slate-600 flex items-center justify-center"
                    >
                  ...
                  <span class="text-lg font-black text-slate-700">{{
                    dashboardData.alerts.expiring_placements
                  }}</span>
                </div>
              </div>
            </n-card>
            ...
            <n-card
              title="Aktivitas Rekrutmen Terbaru"
              class="shadow-sm rounded-xl border border-slate-100"
            >
              <template #header-extra>
