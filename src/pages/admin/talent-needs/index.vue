...
const filteredTalentNeeds = computed(() => {
  ...
  return allTalentNeeds.value.filter((item) => {
    return [item.projectKegiatan, item.bidang, item.picHr, item.status, item.urgensi]
      .join(' ')
      .toLowerCase()
      .includes(keyword)
  })
