  ...
  if (formData.value.applied_roles?.length) {
    const aiChildren = formData.value.applied_roles
      .map((r: any) => {
        ...
        const matched = allRoles.value?.find(
          (ar: any) => ar.name.toLowerCase() === name.toLowerCase(),
        )
        const val = matched
          ? matched.id
          : `NEW_ROLE:${typeof r === 'object' ? r.sector : 'Undefined'}:${name}`
    ...
    const mainGroups = sectors.value
      ...
      .map((sector: any) => {
        const rolesForSector = allRoles.value
          ...
          .sort((a: any, b: any) => a.name.localeCompare(b.name))
        const activeRoles = rolesForSector
          .filter((r: any) => r.is_active)
          .map((role: any) => ({ label: role.name, value: role.id, disabled: false }))
        const inactiveRoles = rolesForSector
          .filter((r: any) => !r.is_active)
          .map((role: any) => ({ label: role.name, value: role.id, disabled: true }))
        return {
          type: 'group',
          label: sector.name,
          key: sector.id,
...
const provinsiOptions = computed(() => {
  return (
    provQuery.data.value?.data?.list?.map((p: any) => ({
      label: p.name,
      value: p.id,
    })) || []
  )
...
const kabupatenOptions = computed(() => {
  return (
    kabQuery.data.value?.data?.list?.map((k: any) => ({
      label: k.name,
      value: k.id,
    })) || []
  )
...
// Serialize a date as a local YYYY-MM-DD string to avoid timezone shifts
const formatDateForServer = (date: string | Date | null | undefined) => {
  if (!date) return null
...
const startEditing = () => {
  ...
  if (profile.value) {
    ...
    formData.value = {
      ...
      provinsi_id: profile.value.kabupaten_kota_id?.includes('.')
        ? profile.value.kabupaten_kota_id.split('.')[0]
        : null,
...
const handleAiCVUpload = (data: { file: UploadFileInfo }) => {
  if (data.file.file) {
    ...
    upload(
      { file: data.file.file, skipParsing: false },
      {
        onSuccess: () => {
...
const handleRegularCVUpload = (data: { file: UploadFileInfo }) => {
  if (data.file.file) {
    ...
    upload(
      { file: data.file.file, skipParsing: true },
      {
        onSuccess: () => {
...
const submitUpload = () => {
  ...
  if (fileList.value.length > 0 && currentFile) {
    upload(
      { file: currentFile, skipParsing: !isAiEnabled.value },
      {
        onSuccess: () => {
          message.success(
            isAiEnabled.value
...
const onConfirm = async () => {
  try {
    ...
    const body = {
      ...formData.value,
      birthdate: formData.value.birthdate ? formatDateForServer(formData.value.birthdate) : null,
      unavailable_until: formData.value.unavailable_until
        ? formatDateForServer(formData.value.unavailable_until)
        : null,
    }
...
const currentStep = computed(() => {
  ...
  // If AI is enabled
  const isParsingOrWaiting =
    cv &&
    (cv.status === 'UPLOADED' || cv.status === 'PARSING' || cv.status === 'FAILED') &&
    cv.parsed_data === null
...
watch(
  () => cvQuery.data.value?.data?.parsed_data,
  (newData) => {
    if (newData) {
      ...
      if (
        ...
        typeof parsed.kabupaten_kota === 'string' &&
        parsed.kabupaten_kota.includes('.')
      ) {
...
<template>
  <CandidateLayout
    :container-class="
      ['UPLOAD', 'PARSING'].includes(currentStep)
      ...
      <n-spin :show="isLoadingProfile || isUploadingPicture">
        ...
        <div v-if="currentStep === 'VIEW'">
          <div class="relative">
            ...
            <div class="relative mt-0">
              ...
              <div
                :class="{ 'blur-sm select-none pointer-events-none grayscale-40': isProfileLocked }"
              >
                ...
                <div class="space-y-12">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-12">
                      ...
                      <div class="space-y-1">
                        ...
                        <p class="text-[17px] font-bold text-gray-800">
                          ...
                          <span v-if="age" class="text-gray-400 font-medium ml-2"
                            >({{ age }} Years Old)</span
                          >
                        </p>
                      </div>
                  ...
                  <section>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-y-8 gap-x-12">
                      <div class="space-y-1">
                        ...
                        <p class="text-[17px] font-bold text-gray-800">
                          {{
                            kabupatenOptions.find(
                              (k: any) => k.value === profile?.kabupaten_kota_id,
                            )?.label || 'Not Specified'
                          }}
                        </p>
                      </div>
                      <div class="space-y-1">
                        ...
                        <p class="text-[17px] font-bold text-gray-800">
                          {{
                            provinsiOptions.find(
                              (p: any) => p.value === profile?.kabupaten_kota_id?.split('.')[0],
                            )?.label || '-'
                          }}
                        </p>
                      </div>
                  ...
                  <section>
                    ...
                    <div class="flex items-center gap-4">
                      ...
                      <template v-else>
                        <n-tag type="warning" round strong size="large"> ✗ Unavailable </n-tag>
                        <div class="space-y-0.5">
                          <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide">
                            Unavailable Until
                          </p>
                          <p class="text-[15px] font-bold text-gray-800">
                            {{ formatDate(profile.unavailable_until) }}
                          </p>
                        </div>
                      </template>
                    </div>
          ...
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
              <h2 class="text-xl font-bold tracking-tight text-white">
                AI Parsing - Auto Fill Form
              </h2>
              <p class="text-gray-400 text-sm mt-1 leading-relaxed">
                Upload your CV and let AI automatically fill in all your information. Save time and
                get accurate data extraction instantly!
              </p>
              <p v-if="cvTemplateUrl" class="text-gray-300 text-sm mt-2">
                ...
                <a
                  :href="cvTemplateUrl"
                  target="_blank"
                  class="underline font-bold text-white hover:text-blue-300 transition-colors"
                  >CV Template.docx</a
                >
                Here
              </p>
            ...
            >
              ...
              >
                ...
                <div v-if="fileList.length === 0" class="flex flex-col items-center w-full">
                  <svg
                    class="w-7 h-7 text-gray-300 mb-3"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    stroke-width="1.8"
                  ...
                  >
                    <path
                      d="M5 19h14a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2H9L7 3H5a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2z"
                    />
                  </svg>
                ...
                <div v-else class="flex flex-col items-center justify-center py-2">
                  ...
                  <p class="text-white font-semibold text-sm max-w-md truncate">
                    {{ fileList[0]?.name }}
                  </p>
        ...
        <div v-if="currentStep === 'PARSING'" class="text-white">
          <div class="p-16 text-center max-w-xl mx-auto">
            <div
              v-if="cvQuery.data.value?.data?.status === 'FAILED'"
              class="flex flex-col items-center"
            >
              <div
                class="w-20 h-20 bg-red-500/10 text-red-500 rounded-3xl flex items-center justify-center mb-6"
              >
              ...
              <p class="text-gray-400 mt-2 mb-8">
                We were unable to parse your CV automatically. This could be due to a temporary
                service interruption or document format issues. You can fill in your details
                manually to complete your profile.
              </p>
              <div class="flex justify-center gap-4">
                <n-button type="primary" color="#0014B2" @click="startEditing" class="font-bold">
                  Fill Profile Manually
                </n-button>
                <n-button ghost @click="showUpdateFlow = false"> Cancel </n-button>
              </div>
            </div>
            ...
            <div v-else class="flex flex-col items-center">
              ...
              <div
                class="w-full bg-gray-800 rounded-full h-3.5 overflow-hidden relative shadow-inner mt-6 mb-3"
              >
                <div
              ...
              <!-- Progress Details -->
              <div
                class="flex justify-between items-center w-full px-1 text-sm font-bold text-gray-400 mb-4"
              >
              ...
              <div class="mt-8 flex gap-2">
                <div class="w-2.5 h-2.5 bg-blue-500 rounded-full animate-bounce"></div>
                <div
                  class="w-2.5 h-2.5 bg-blue-500 rounded-full animate-bounce [animation-delay:-0.15s]"
                ></div>
                <div
                  class="w-2.5 h-2.5 bg-blue-500 rounded-full animate-bounce [animation-delay:-0.3s]"
                ></div>
              </div>
            </div>
          </div>
        ...
        <div v-if="['REVIEW', 'EDIT'].includes(currentStep)">
              ...
              <p class="text-gray-400 mt-1 text-sm">
                {{
                  currentStep === 'EDIT'
                    ...
                    : 'Please verify the information extracted by our AI assistant.'
                }}
              </p>
            </div>
          ...
          <div
            v-if="isAiEnabled && (currentStep === 'REVIEW' || cvLinkData?.data?.url)"
            ...
            <div class="flex items-center gap-4 z-10">
              ...
              <div class="text-left">
                ...
                <p class="text-sm text-gray-300 mt-1 leading-relaxed max-w-xl">
                  We’ve pre-filled the form for you. Please double-check the information to ensure
                  ...
                  <n-grid :cols="2" :x-gap="16">
                    <n-gi>
                      <n-form-item label="Email Address">
                        <n-input
                          :value="profile?.email || authStore.user?.email || ''"
                          disabled
                          placeholder="Auto from regist"
                        >
                          <template #prefix
                            ><n-icon :component="ExternalLink" class="text-gray-400"
                          /></template>
                        </n-input>
                      </n-form-item>
                    </n-gi>
                    <n-gi>
                      <n-form-item label="Phone Number">
                        ...
                        >
                          <template #prefix
                            ><n-icon :component="User" class="text-gray-400"
                          /></template>
                        </n-input>
                      </n-form-item>
                    </n-gi>
              ...
              <section>
                ...
                <div class="border border-gray-200 rounded-xl p-5 space-y-4">
                  <n-form-item label="University / School">
                    <n-input
                      v-model:value="formData.school_university"
                    ...
                    <n-gi>
                      <n-form-item label="City / Regency">
                        <n-select
                          ...
                          placeholder="Select your city or regency"
                          filterable
                          clearable
                        />
                      </n-form-item>
              ...
              <section>
                ...
                <div class="border border-gray-200 rounded-xl p-5 space-y-4">
                  ...
                  <n-grid :cols="2" :x-gap="16">
                    ...
                    <n-gi>
                      <n-form-item label="Unavailable Until">
                        <n-date-picker
                          v-model:value="formData.unavailable_until"
                          type="date"
                          class="w-full"
                          clearable
                  ...
                  <n-form-item label="Portfolio Link">
                    <n-input
                      v-model:value="formData.portofolio_link"
                      placeholder="Enter your portfolio URL"
                    >
                      <template #prefix
                        ><n-icon :component="Link" class="text-gray-400"
                      /></template>
                    </n-input>
                  </n-form-item>
                    ...
                    <n-spin :show="isUploadingRegular" class="w-full">
                      ...
                      <div v-if="cvLinkData?.data?.url" class="w-full flex flex-col gap-2">
                        ...
                        >
                          ...
                          <div
                            @click.stop="triggerRegularCVUpload"
                        ...
                        >
                          <div class="flex flex-col items-center">
                            ...
                            <p class="text-gray-400 text-xs mt-1 mb-4">
                              Supported file types: PDF, JPG, GIF
                            </p>
                            <span
