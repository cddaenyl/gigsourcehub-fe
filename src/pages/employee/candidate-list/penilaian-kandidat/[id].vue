...
const { review, isLoading: isReviewLoading, isError: isReviewError, error: reviewError } = useReviewDetail(reviewId)
...
<template>
  <EmployeeLayout>
    <div class="mx-auto space-y-6 max-w-4xl">
      ...
      <!-- Loading State -->
      <div v-if="isReviewLoading || isUserLoading || isOnboardingLoading" class="flex justify-center items-center py-20">
        <n-spin size="large" />
      </div>

      <!-- Error State -->
      <div v-else-if="isReviewError" class="text-center py-20 bg-white rounded-lg border border-gray-200 p-6">
      ...
      <div v-else-if="review" class="space-y-6">
        ...
        <n-card class="shadow-sm border border-slate-200">
          <div class="flex items-center gap-4">
            ...
            />
            <div class="flex-1">
              <h2 class="text-lg font-bold text-slate-800">{{ candidateUser?.name || 'Kandidat' }}</h2>
        ...
        <!-- Questionnaire Indicators Cards (Read-only) -->
        <n-card
        ...
        >
          <div class="space-y-4">
            ...
            <!-- List of answers for this indicator -->
          ...
          <div class="space-y-4">
            ...
            <div class="space-y-2">
              ...
              <div
                ...
                :class="[
                  review.final_recommendation === 'RECOMMENDED'
                    ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
                    : 'bg-rose-50 text-rose-700 border-rose-200'
                ]"
              >
                {{ review.final_recommendation === 'RECOMMENDED' ? 'Direkomendasikan' : 'Tidak Direkomendasikan' }}
              </div>
            </div>

            <div class="space-y-2 mt-4" v-if="review.notes">
              <span class="text-xs font-bold text-slate-600 block">Catatan Tambahan</span>
              <div class="bg-slate-50 p-4 rounded-lg border border-slate-200 text-xs text-slate-700 leading-relaxed whitespace-pre-wrap">
                {{ review.notes }}
              </div>
            </div>
          </div>
