...
import { createJobRoleApi, getSectorsApi } from '@/services/job-role.service'
  ...
  formRef.value?.validate(async (errors: any) => {
    if (!errors) {
      ...
      try {
        ...
        message.success('Posisi berhasil ditambahkan')
        router.push('/superadmin/posisi')
      } catch (err: any) {
        message.error(err.message || 'Gagal menambahkan posisi')
      } finally {
